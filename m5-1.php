<?php
    ...
    if (!empty($_POST["name"])&&!empty($_POST["comment"])&&!empty($_POST["pass"])){//フォームに名前とコメントが書かれているかならば
        $newnumber=$_POST["new"];//＄newnumberはフォームからは見えない編集対象番号を表示することで編集モードというのを認識させる
         if(!empty($newnumber)){//$newnumberに入力されていれば
            ...
            $stmt->bindParam(':id', $newnumber, PDO::PARAM_INT);
            $stmt->execute();
    ...
    }elseif(!empty($_POST["delete"])&&!empty($_POST["pass"])){//フォームに名前とコメントが入ってなく削除機能欄が空でない時
  
            $sql = 'delete from bbs where id=:id AND passward=:passward';
            $stmt = $pdo -> prepare($sql);
    ...
    }elseif(!empty($_POST["edit"]&&!empty($_POST["pass"]))){//フォームに名前とコメントが入ってなく削除機能欄が空で編集機能欄が空でないとき
