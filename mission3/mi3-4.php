<?php
        ...
        $newnumber="";//編集番号一致した時フォームに編集対象を出す際の箱??
        
        
        
          
        if ($_POST["name"]!=NULL&&$_POST["comment"]!=NULL){//フォームに名前とコメントが書かれているかならば
             $newnumber=$_POST["new"];//＄newnumberはフォームからは見えない編集対象番号を表示することで編集モードというのを認識させる
              if($newnumber!=NULL){//$newnumberに入力されていれば
                        ...
                        foreach($lines as $line){
                            $editnum = explode("<>", $line);//投稿番号を変数に入れた
                                ...
                                if($newnumber==$editnum[0]){//編集対象番号と投稿番号が同じなら
                                    fwrite($fp,$newnumber."<>".$name."<>".$comment."<>".$date.PHP_EOL);//対象の投稿番号．新しい名前．新しいコメント
                ...
                $fp = fopen($filename,"a");
                    ...
                    fwrite($fp, $count."<>".$name."<>".$comment."<>".$date.PHP_EOL);//フォームから投稿内容を拾って来る
                    fclose($fp);
            }//else終わり 　ここまででフォーム入力機能or編集機能おわり 
            
        }elseif($_POST["delete"]!=NULL){//フォームに名前とコメントが入ってなく削除機能欄が空でない時
            $lines = file($filename,FILE_IGNORE_NEW_LINES);
            $fp = fopen( $filename, "w");
            ...
            for ($i = 0; $i < count($lines_1); $i++){//$i=0のときcount($lines)はテキストファイルの行数は０より小さいか
                ...
                if ($postnum == $edit){//編集機能で選択した番号とpostnumが一致しているか
